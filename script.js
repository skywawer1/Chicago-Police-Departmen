...
function renderUI() {
    ...
    sidebar.innerHTML += `
        <div style="margin-bottom: 20px;">
            ...
            <select onchange="window.changeMode(this.value)" style="width: 100%; padding: 10px; background: #0f172a; color: var(--accent-blue); border: 1px solid var(--border); border-radius: 4px; font-weight: bold; cursor: pointer; outline: none;">
    ...
    if (currentMode === "JSA") {
        ...
        sidebar.innerHTML += `
            ...
            <button onclick="switchTab('Сотрудники ГЕД')" class="menu-btn">🪪 СОТРУДНИКИ DB/GED</button>
...
// Отрисовка навигации (меню, которое потерялось)
window.renderNav = function() {
    const terminalTabs = ['Главная', 'База Данных', 'JSA Liaison', 'Панель Управления'];
    const nav = document.getElementById('sidebar-menu');
    if (nav) {
        // Убрали очистку innerHTML, чтобы не стереть кнопки из renderUI
...
function switchTab(tab) {
    ...
    if (tab === 'Ожидание одобрения') {
        ...
        if (isCase) {
            ...
            window.renderReportTable(container, caseFilesList);
        }
    }
    else if (tab === 'Постановления JSA') {
        let isJSA = (currentUser.rank === 'JSA' || currentUser.rank === 'ADMIN');
        let htmlContent = `<p class="tab-subtitle" style="color:var(--text-gray); margin-bottom: 20px;">Официальные постановления и судебные ордера от сотрудников Юстиции (JSA).</p>`;
        ...
        if (isJSA) {
            htmlContent += `
                <div class="form-box" style="border-left: 4px solid #eab308; margin-bottom: 30px;">
                    ...
                    <div class="actions">
                        <button onclick="window.submitJSADocument('постановление')" class="btn-primary" style="background:#eab308; color:#000; font-weight:bold;">ОПУБЛИКОВАТЬ ДОКУМЕНТ</button>
                    </div>
                </div>`;
        } else {
            htmlContent += `
        ...
        htmlContent += `<br><hr style="border-color:var(--border); margin: 25px 0;"><h3 style="color:var(--accent-blue); margin-bottom:15px;">АРХИВ ПОСТАНОВЛЕНИЙ</h3>`;
        container.innerHTML += htmlContent;

        let jsaList = reports.filter(r => r.globalType === 'jsa-doc' || r.type === 'постановление');
        window.renderReportTable(container, jsaList);
    }
    else if (tab === 'Обращения к JSA' || tab === 'Написать в JSA') {
        let htmlContent = `
            ...
            <div class="form-box" style="margin-bottom: 30px;">
                ...
                <div class="actions">
                    <button onclick="window.submitJSADocument('обращение')" class="btn-primary">ОТПРАВИТЬ В ЮСТИЦИЮ</button>
                </div>
            </div>`;
        ...
        if (isJSA) {
            container.innerHTML += `
                <div class="form-box" style="border-left: 4px solid #eab308; margin-bottom: 30px;">
                    ...
                    <button onclick="window.submitJSADocument('jsa-news')" class="btn-primary" style="background:#eab308; color:#000; font-weight:bold;">ПУБЛИКАЦИЯ ДЛЯ ВСЕХ</button>
                </div>`;
        }
        ...
        window.renderReportTable(container, newsList);
    }
    else if (tab === 'Заявления от сотрудников') {
        ...
        window.renderReportTable(container, appealsList);
    }
            ...
            if (currentUser.rank === 'ADMIN' || currentUser.rank === 'SGT' || currentUser.rank === 'JSA') {
                strikeBtns = `
                    <div style="margin-top: 10px; display:flex; gap:5px;">
                        <button onclick="window.issueStrike('${email}', 'verbal')" style="flex:1; padding: 5px; font-size:10px; background:#f59e0b; color:#fff; border:none; border-radius:3px; cursor:pointer;">+ УСТНЫЙ</button>
                        <button onclick="window.issueStrike('${email}', 'written')" style="flex:1; padding: 5px; font-size:10px; background:#ef4444; color:#fff; border:none; border-radius:3px; cursor:pointer;">+ ПИСЬМЕННЫЙ</button>
                    </div>`;
            }
        ...
        window.renderReportTable(container, list);
    }
...
function editReportText(id) {
    ...
    document.getElementById('modal-body').innerHTML = `
        ...
        <div class="actions" style="margin-top: 15px;">
            ...
            <button onclick="window.viewReport('${id}')" class="btn-secondary">ОТМЕНА</button>
        </div>`;
}
